...
MANIFEST = os.path.join(ROOT, "research", "render_manifest_v8.json")
...
def intro_frame(t):
    ...
    d.text((80, 700), "OPEN MODELS ARE CLOSING FAST.", font=font("Archivo", 31), fill=WHITE)
    d.text((80, 740), "Each signal includes a visible source/status cue.", font=font("Archivo", 20), fill=(215,225,229))
    d.text((SAFE_X0, 1060), "SOURCE-FIRST EDITORIAL SYSTEM · v8", font=font("Archivo", 17), fill=MUTED)
...
def story_frame(index, local_t):
    ...
    title_font = font("Archivo", 42)
    y = 168 + int((1-p)*24)
    for line in wrap(d, story["headline"].upper(), title_font, 470, 3):
...
def recap_frame(t):
    ...
    d.text((SAFE_X0, 188), "OPEN MODELS", font=font("Archivo", 58), fill=INK)
    d.text((SAFE_X0, 258), "ARE CLOSING FAST.", font=font("Archivo", 58), fill=INK)
...
def main():
    frame_dir = os.path.join(ROOT, "visual", "frames_v8")
    os.makedirs(frame_dir, exist_ok=True)
    frame_count = round(DURATION * FPS)
    print(f"Rendering {frame_count} v8 frames at {FPS}fps…")
    for i in range(frame_count):
        draw_frame(i / FPS).save(os.path.join(frame_dir, f"f{i:05d}.png"))
        if i % 300 == 0:
            print(f"frame {i}/{frame_count}")
    output = os.path.join(ROOT, "visual", "visual_master_v8.mp4")
    subprocess.run([
